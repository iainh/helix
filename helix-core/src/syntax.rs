...
mod test {
    ...
    fn test_bracket_context_fallback_to_code() {
        ...
        assert!(matches!(
            context,
            BracketContext::Code | BracketContext::Unknown
        ));
    }
    ...
    fn test_auto_pairs_with_context_rust_code_vs_string() {
        ...
        let pos_code = 18;
        let ctx_code =
            lang_data.bracket_context_at(syntax.tree(), source.slice(..), pos_code, &LOADER);
        ...
        let state_code =
            AutoPairState::with_contexts(&source, &selection_code, &pairs, &contexts_code);
        ...
        let pos_string = 14;
        let ctx_string =
            lang_data.bracket_context_at(syntax.tree(), source.slice(..), pos_string, &LOADER);
        assert_eq!(
            ctx_string,
            BracketContext::String,
            "Position 14 should be String"
        );

        let selection_string = Selection::single(pos_string, pos_string + 1);
        let contexts_string = vec![ctx_string];
        let state_string =
            AutoPairState::with_contexts(&source, &selection_string, &pairs, &contexts_string);

        let result_string = hook_with_context(&state_string, '(');
        assert!(
            result_string.is_some(),
            "Should return transaction even when blocked"
        );
    ...
    fn test_auto_pairs_with_context_rust_comment() {
        ...
        let pos_comment = 5;
        let ctx =
            lang_data.bracket_context_at(syntax.tree(), source.slice(..), pos_comment, &LOADER);
